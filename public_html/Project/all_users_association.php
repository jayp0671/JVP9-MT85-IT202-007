...
$usernameFilter = isset($_GET['username_filter']) ? $_GET['username_filter'] : ''; // New filter for username

// Query to retrieve data from your table with filtering and sorting
$query = "SELECT Recipes.*, Users.username, 
          (SELECT COUNT(*) FROM Recipes AS r WHERE r.title = Recipes.title AND r.user_id IS NOT NULL) AS title_count
          FROM Recipes 
...
if ($result->num_rows > 0) {
    ...
    echo " Username Filter: <input type='text' name='username_filter' value='$usernameFilter'>";
    echo "<button type='submit'>Apply</button>";
    echo "</form>";

    echo "<h2>Data List ($totalItems Items):</h2>";
    echo "<table border='1'>";
    echo "<tr><th>Title</th><th>Ingredients</th><th>Instructions</th><th>Servings</th><th>Source</th><th>Username</th><th>Title Count</th><th>Actions</th></tr>";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td><a href='user_profile.php?id=" . $row["user_id"] . "'>" . $row["username"] . "</a></td>";
        echo "<td>" . $row["title_count"] . "</td>";

        

        // Add links for actions
        echo "<td class='action-links'>";
        echo "<a href='view_recipe_all.php?id=" . $row["id"] . "'>View</a>";
        echo "<a href='delete_recipe_all.php?id=" . $row["id"] . "'>Delete</a>";

        // Link/button to remove all associations for the matching user
        echo "<a href='remove_all_user_associations.php?username=" . $row["username"] . "'>Remove All Associations</a>";
